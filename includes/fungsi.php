...
$insertsa = mysql_query("insert into `useraura`(user,password,email,level,tipe) values('superadmin','$passwordsa','reky@teamworks.co.id','Administrator','aktif')");
}
...
echo '<div class="error"><b>ERROR :</b> Username atau Password Salah, Lupa Pasword Hubungi admin</div>';
}
}
...
function cekjmlujian2 ($idmapel){
$query = "SELECT * FROM ujian WHERE idmapel = '$idmapel'";
$hasil = mysql_query($query);
$total  = mysql_num_rows($hasil);
return $total;
}
...
echo '<div class="error"><b>ERROR :</b> Tipe Ujian bukan Latihan , Score / Nilai tidak akan disimpan sebagai Nilai Baru.</div>';
}
}
}
